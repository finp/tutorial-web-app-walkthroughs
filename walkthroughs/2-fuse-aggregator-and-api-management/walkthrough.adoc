...
. Log in to the link:{launcher-url}/launch/wizard/{walkthrough-namespace}[{launcher-service}, window="_blank", id="wt2_section_1-1_1"] wizard.
...
[type=verification]
====
Check that the following pods are running in the link:{openshift-host}/console/project/{walkthrough-namespace}[OpenShift Project, window="_blank", id="id="wt2_section_1-1_2""]:
...
. Navigate to the link:{route-fuse-flights-aggregator-host}/camel/flights[Flights Endpoint, window="_blank", id="id="wt2_section_1-2_1"].
...
// TODO placeholders for product names
// TODO project name
. Log in to the link:{che-url}[{code-ready-service}, window="_blank", id="wt2_section_2_1"] Dashboard
...
After waiting for the build and deployment to complete, check link:{route-fuse-flights-aggregator-host}/camel/flights[Flights Endpoint, window="_blank", id="wt2_section_2_2"]. Are more than 8 flights displayed?
...
. Open the link:{api-management-url}[{3Scale-ProductName} Login screen, window="_blank", id="wt2_section_3-1_1"].
